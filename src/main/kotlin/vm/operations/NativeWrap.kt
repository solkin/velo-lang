...
/**
 * Wraps a JVM object (from RefRecord) into a full Velo class instance.
 * 
 * This operation:
 ...
 * 4. Returns a RefRecord with CLASS kind
 * 
...
class NativeWrap(
    ...
    override fun exec(pc: Int, ctx: VMContext): Int {
        ...
        // Pop the JVM object from the stack (it's in a RefRecord from NativeInvoke)
        ...
        while (classFrame.pc < classFrame.ops.size) {
            ...
            if (cmd is NativeConstructor) {
                ...
                val nativeRecord = RefRecord.native(jvmObject, ctx)
                classFrame.subs.push(nativeRecord)
                classFrame.pc++
                continue
            }
            
            // Stop before Instance - we'll create our own RefRecord
            if (cmd is Instance) {
                break
            }
        ...
        // Create RefRecord with the class frame and native index
        val record = RefRecord.classInstance(classFrame, nativeInstanceIndex, ctx)
