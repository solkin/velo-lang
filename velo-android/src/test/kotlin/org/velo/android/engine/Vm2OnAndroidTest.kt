...
/**
 * Verifies the clean-room **velo-vm2** backend against the Android app's own
 ...
 *
 *  - [runsAndroidNativesOnVm2] proves vm2 links and executes the app's
 *    Terminal/Time/FileSystem natives end to end.
 *  - [threadedActorsOnVm2] proves pluggable actor placement: with a pooled
...
class Vm2OnAndroidTest {
    ...
    private fun run(source: String, threaded: Boolean): String {
        val velFile = File.createTempFile("vm2-android", ".vel").apply { writeText(source) }
        try {
            ...
            try {
                ...
                if (threaded) runtime.actorPlacement { PooledDispatcherFactory() }
                runtime.run(program)
            } finally {
    ...
    @Test
    fun runsAndroidNativesOnVm2() {
        val dataFile = File.createTempFile("vm2-e2e", ".txt").apply { delete() }
        ...
        try {
            val source = """
                ...
                Time time = new Time();
                time.sleep(1);
                fs.write("$path", "hello-vm2");
                term.print(fs.read("$path"));
            """.trimIndent()
            assertEquals("hello-vm2", run(source, threaded = false))
        } finally {
            dataFile.delete()
        }
    }
