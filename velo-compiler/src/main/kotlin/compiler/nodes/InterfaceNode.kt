 ...
 * (Go-style). Concrete classes are matched by name through [TypeRegistry] (a
...
data class InterfaceType(
    ...
    override fun sameAs(type: Type): Boolean = when (type) {
        ...
        is ClassType -> {
            val provided = TypeRegistry.classMethods[type.name]
            provided != null &&
...
/**
 * Compile-time table of every Velo class's method signatures, keyed by class
 * name. It is the source of truth for structural interface checks, which run
 * inside [InterfaceType.sameAs] where no [Context] is available. Populated as
 * each class compiles and reset at the start of every compilation
 * ([compiler.VeloCompiler]); compilation is single-threaded per program.
 */
object TypeRegistry {
 ...
 */
fun checkTypeArgBounds(className: String, typeArgs: List<Type>) {
    val bounds = TypeRegistry.classBounds[className] ?: return
