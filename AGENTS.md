...
Velo Lang is a functional, strictly-typed, compilable programming language that runs on a minimal stack-based virtual machine (51 operations). It is designed to be embeddable in JVM applications.
...
```
velo-core      Shared contracts: Op (51 opcodes), VmType, Bytecode (.vbc read/write),
...
1. **VM** — `VM.kt` loads frames and runs; `VMExecutor.kt` drives the loop; `Interpreter.kt` is the single `when`-dispatch over all 51 ops.
