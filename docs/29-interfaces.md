...
`Self` is resolved entirely at compile time to the receiver's type at each call site, so `c.bump()` is a `Counter` and the chain type-checks. Use `Self` only as a **return type** — that is what keeps structural satisfaction sound. The compiler will also accept `Self` in a parameter position, but such a parameter is unusable: any access to it fails with *"Property … of Self is not supported"*, so don't declare `Self` parameters.
